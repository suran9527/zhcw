                  ...
                  <template v-else-if="item.colKey=='img'">
                    <template v-if="formData[item.colKey].length!=0&&Array.isArray(formData[item.colKey])" v-for="(url,index) in formData[item.colKey]">
                      <div style="position: relative">
                        <el-image
                          style="width:110px;height:110px;margin-right: 20px"
                          :src="url">
                        </el-image>
                      ...
                      <t-upload v-if="formData.img.length==index+1&&formData[item.colKey].length!=3"
                                ...
                                accept="image/*"
                                multiple
                                :max="3-formData[item.colKey].length"
                      ></t-upload>
                    </template>
      ...
      <t-row :gutter="[16,16]" v-show="!loading">
        ...
        <template v-for="item in data">
          <t-col :lg="4"
                 :xs="6"
                 :xl="3">
            <t-card :cover="Array.isArray(item.img)?item.img[0]:item.img">
              <template #avatar>
              <t-row>
...
import {getNewsList,addNews,delNews,editNews} from '@/api/news';
export default {
  name: 'Notice',
  components: {
    Icon
  ...
  methods: {
    ...
    getNewsList(){
      ...
      getNewsList({
        ...
        for(let i in res.data) {
          res.data[i].title.length>5?this.data[i].newTitle=res.data[i].title.substring(0,5)+'....':''
    ...
    addNews(){
      ...
      for(let i in this.files){
        arr.push(this.files[i].response.data.links.thumbnail_url)
      }
    ...
    clickHandler(data) {
      if(data.content=="编辑") {
        ...
        delNews({id:this.index.id}).then(res=>{
          if(res.code==211){
            this.$message.success({ content: res.msg, duration: 2000 })
            this.visible=true
            this.getNewsList();
    ...
    onConfirmAnother1(){
      console.log(this.files)
      for(let i in this.files){
        this.formData.img.push(this.files[i].response.data.links.thumbnail_url)
      }
      Array.isArray(this.formData.img)?this.formData.img=this.formData.img.join(','):''
      editNews(this.formData).then(res=>{
        console.log(res)
          if(res.code==211){
            this.$message.success({ content: res.msg, duration: 2000 })
