        ...
        <t-menu width="232px" class={this.menuCls} theme={this.theme} value={this.active} collapsed={this.collapsed}>
          {this.showLogo && (
            <span
              ...
              onClick={() => this.handleNav('/dashboard/home')}
            >
              {this.collapsed ? (
                // <Logo class={`${prefix}-side-nav-logo-t-logo`} />
				<span style='color: #4582e6;letter-spacing: 2px;font-size: 24px;font-family:"微软雅黑","楷体";'>智</span>
              ) : (
                // <LogoFull class={`${prefix}-side-nav-logo-tdesign-logo`} />
				<span style='color: #4582e6;letter-spacing: 4px;font-size: 30px;font-family:"楷体","微软雅黑";'>智慧村务</span>
              )}
            </span>
          )}
          <menu-content navData={this.menu}></menu-content>
          <span slot="operations" class="version-container">
            {!this.collapsed && '智慧村务'} {pgk.version}
          </span>
        </t-menu>
